---
title: 'PostgreSQL Storage & Architecture Decisions'
---
...
If storage lies about durability → PostgreSQL breaks.
...
👉 **Default choice for production PostgreSQL**.
...
👉 **Avoid for production OLTP workloads**.
...
👉 **Only for highly mature teams**.

---

## 4. Kubernetes StorageClass Selection

You don’t pick disks directly. You pick a **StorageClass**.
...
## 5. PostgreSQL on Kubernetes – Full Architecture
...
```
App → Primary Postgres → Replica Postgres
```
...
## 9. Azure Managed PostgreSQL – Why Many Teams Choose It
...
> “PostgreSQL requires block storage for correctness and performance. On Kubernetes, this means block-backed persistent volumes such as Azure Managed Disks. Network file systems are risky for production workloads. For cloud-first environments, Azure Managed PostgreSQL is often preferred because it offloads storage, backups, patching, and HA, allowing platform teams to focus on reliability and security rather than database internals.”
