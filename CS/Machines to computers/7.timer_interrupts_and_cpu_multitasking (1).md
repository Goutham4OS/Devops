---
title: 'Timer Interrupts & Multitasking – How the CPU Is Safely Stopped and Switched'
---

# Timer Interrupts & Multitasking – How the CPU Is Safely Stopped and Switched
...
When time is up → signal goes HIGH.
...
If no interrupt → continue program  
If interrupt → special interrupt path
...
- More memory access → slower execution
...
- CPU clock → engine RPM
- Timer interrupt → traffic police whistle
- OS scheduler → traffic controller
...
> “A hardware timer raises an interrupt signal; the CPU checks interrupts between instructions, jumps to the OS interrupt handler, and the scheduler safely saves and restores process state to enable multitasking.”
...
This is one of the most common confusions, so let’s make it **absolutely clear**.
...
👉 Think of timer interrupt as:
> “Traffic police whistle deciding who uses the road”
